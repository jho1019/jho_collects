...
.EXAMPLE
  ...
  pwsh scripts/schedule_backup.ps1 -Time 01:30 -KeepDays 60 `
       -OutDir "$env:OneDrive\jho_collects_backups"

.EXAMPLE
  pwsh scripts/schedule_backup.ps1 -Unregister
#>
[CmdletBinding()]
param(
  [string] $Time = "02:00",
...
Write-Host "  backup : $repo\scripts\backup.py --keep $KeepDays$(if($OutDir){" --out `"$OutDir`""})"
